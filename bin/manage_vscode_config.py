...
class VSCodeConfigManager:
        ...
        try:
            ...
            for derivative_file in derivative_files:
                if derivative_file.exists():
                    derivative_content = self.clean_vscode_json(
                        derivative_file.read_text()
                    )
            ...
            print(
                f"Warning: JSON parsing failed, falling back to text-based merge: {e}"
            )
            return self._merge_settings_text_based(base_file, derivative_files)

    def _merge_settings_text_based(
        self, base_file: Path, derivative_files: List[Path]
    ) -> str:
    ...
    def merge_keybindings(self, base_file: Path, derivative_files: List[Path]) -> str:
        ...
        try:
            ...
            if not isinstance(base_keybindings, list):
                print("Warning: Base keybindings is not an array, converting")
                base_keybindings = []
            ...
            print(
                f"Warning: JSON parsing failed for keybindings, falling back to text merge: {e}"
            )
            return self._merge_keybindings_text_based(base_file, derivative_files)

    def _merge_keybindings_text_based(
        self, base_file: Path, derivative_files: List[Path]
    ) -> str:
        ...
        try:
            ...
            for derivative_file in derivative_files:
                if derivative_file.exists():
                    ...
                    if derivative_content.startswith(
                        "{"
                    ) and derivative_content.endswith("}"):
    ...
    def create_merged_config(
        self, config_type: str, derivative: Optional[str] = None
    ) -> Optional[Path]:
    ...
    def deploy_vscode_configs(self):
        ...
        all_derivatives = set(
            derivatives.get("settings", []) + derivatives.get("keybindings", [])
        )
        ...
        for variant, config_path in vscode_paths.items():
            ...
            if variant_lower in derivative_mapping:
                ...
                for derivative in all_derivatives:
                    if (
                        derivative.lower().replace(" ", "").replace("-", "")
                        == variant_lower
                    ):
            ...
            if temp_settings:
                ...
                try:
                    ...
                    print("  ✓ Deployed settings.json")
                    temp_settings.unlink()  # Clean up temp file
                except IOError as e:
                    print(f"  ✗ Failed to deploy settings: {e}")

            # Deploy keybindings
            temp_keybindings = self.create_merged_config(
                "keybindings", variant_derivative
            )
            if temp_keybindings:
            ...
            # Deploy snippets
            snippets_src = self.vscode_dir / "snippets"
            if snippets_src.exists():
                snippets_dst = config_path / "snippets"
                try:
                    snippets_dst.mkdir(parents=True, exist_ok=True)
                    for snippet_file in snippets_src.glob("*.json"):
                        shutil.copy2(snippet_file, snippets_dst / snippet_file.name)
                        print(f"  ✓ Deployed snippets/{snippet_file.name}")
                except IOError as e:
                    print(f"  ✗ Failed to deploy snippets: {e}")
    ...
    def remove_vscode_configs(self):
        ...
        for variant, config_path in vscode_paths.items():
            ...
            if snippets_src.exists():
                snippets_dst = config_path / "snippets"
                for snippet_file in snippets_src.glob("*.json"):
                    target = snippets_dst / snippet_file.name
                    if target.exists():
                        try:
                            target.unlink()
                            print(f"  ✓ Removed snippets/{snippet_file.name}")
                        except IOError as e:
                            print(
                                f"  ✗ Failed to remove snippets/{snippet_file.name}: {e}"
                            )
